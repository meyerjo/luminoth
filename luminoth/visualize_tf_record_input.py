...
def run(config, target='', cluster_spec=None, is_chief=True, job_name=None,
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        try:
            print('Staart Corrdinate')
            while not coord.should_stop():
                before = time.time()
                # _, train_loss, step, filename = sess.run([
                #     #train_op, total_loss, global_step,
                #     train_filename
                # ], options=run_options)
                image, filename, boxes = sess.run([
                    #train_op, total_loss, global_step,
                    train_image, train_filename, train_bboxes
                ], options=run_options)
                print(filename, image, image.shape, boxes)
                from PIL import Image
                im = Image.fromarray(image.astype(dtype=np.uint8), 'RGB')
                im.show()
                exit(-1)
